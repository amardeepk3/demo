    ...
    $.fn.gridEditor = function (options) {
        ...
        self.each(function (baseIndex, baseElem) {
            ...
            function createColControls() {
                ...
                canvas.find('.column').each(function (ind) {
                    var col = $(this);
                    ...
                    createTool(drawer, 'Add Form ele', 'ge-add-formele', 'fa fa-plus-circle', function () {
                        // var row = createRow();
                        alert(drawer[0].parentNode.children[2]);
                        console.log(drawer[0].parentNode.children[2]);
                        var row = createFormEle();
                        col.append(row);
                        // row.append(createColumn(6)).append(createColumn(6));
                        init();
                    });

                    createDropDownTool(drawer, 'Add modal', 'ge-add-formele', 'fa fa-plus-circle',ind, function () {
                        // var row = createRow();
                        console.log(drawer[0].parentNode);
                    
                        // const id = `${drawer[0].clientWidth}-${drawer[0].clientHeight}` 
                        // col[0].classList.add(id);
                        // var row = createModel(col[0].className.trim());
                        // col.append(row);
                        // row.append(createColumn(6)).append(createColumn(6));
                        init();
                    });
            ...
            function createDropDownTool(drawer, title, className, iconClass, ind, eventHandlers) {
                // var tool = $('<a title="' + title + '" class="' + className + '"><i class="' + iconClass + '"></i></a>')
                //     .appendTo(drawer)
                //     ;
                
                const id = `${ind}-${drawer[0].offsetParent.offsetHeight}-${drawer[0].offsetParent.offsetWidth}-${drawer[0].clientWidth}-${drawer[0].clientHeight}` ;
                const btn = '<button type="button" class="btn btn-primary" data-toggle="modal" id="'+id+'" data-rowid="'+id+'" data-target="#exampleModal">'+
                        'Launch demo modal'+
                    '</button>';
                var tool = $(btn).appendTo(drawer);
                if (typeof eventHandlers == 'function') {
                    tool.on('click', eventHandlers);
                }
                if (typeof eventHandlers == 'object') {
            ...
            function createModel(col){
            //    const str = '< !--Button trigger modal-- >'+
                  ...
                  '<div class="modal-dialog" role="document">'+
                    '<div class="modal-content">'+
                      ...
                      '<div class="modal-body">'+
                        '<button type="button" class="btn btn-secondary" onclick="addTextForm('+`'${col}'`+')">add text form</button>'+
                      '</div>'+
                ...
                if (!contentArea.hasClass('active')) {
                    ...
                    var configuration = $.extend(
                        ...
                        (settings.tinymce && settings.tinymce.config ? settings.tinymce.config : {}),
                        {
                            ...
                            //     // editor.activeEditor.formatter.apply('custom_format')
                            // },
                            ...
                            templates: [
                                ...
                                //   },
                                {
                                    ...
                                    description: "Adds an inputelement."
                                }
                            ],
                            // extended_valid_elements:"Headers",
                            // formats: {
                            //     custom_format: { block: 'h1', attributes: { title: 'Headers' }, styles: { color: 'red' } }
                            // },
                            style_formats: [
                                ...
                                {title: 'form label', inline: 'label', classes: 'example2'},
                                {title: 'Table styles'},
